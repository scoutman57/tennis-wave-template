			...
			clickHandler = function(e){
				...
				if ( $submenu.length && (sf.op.useClick || !follow) ){
					...
					if (!$submenu.is(':visible')){
						$.proxy(over,$a.parent('.sfHolder').parent('li'))();
					} else if (sf.op.useClick && follow) {
						$.proxy(out,$a.parent('.sfHolder').parent('li'),e)();
					}
				}
			},
			addArrows = function($li,o){
				if (o.autoArrows) {
					$li.children('.sfHolder').children('a').each(function() {
						addArrow( $(this) );
					});
				}
			},
		...
		hideSuperfishUl : function(){
			...
			var $ul = $('li.'+o.hoverClass,this).add(this).not(not)
					.find(' > .sfHolder>ul').stop().animate(o.animationOut,o.speedOut,function(){
						$ul = $(this);
						$ul.parent().parent().removeClass(o.hoverClass);
						o.onHide.call($ul);
						if (sf.op.useClick){
